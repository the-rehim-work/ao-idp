...
const BORDER = 'rgba(94,234,212,0.3)'
const GLOW = '0 0 8px rgba(94,234,212,0.6), 0 0 20px rgba(94,234,212,0.25)'
const TITLE_GLOW = '0 0 8px rgba(94,234,212,0.9), 0 0 20px rgba(94,234,212,0.5), 0 0 40px rgba(94,234,212,0.2)'
...
export default function LoginPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', fontFamily: '"JetBrains Mono", "Courier New", monospace', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', background: 'repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(94,234,212,0.015) 2px, rgba(94,234,212,0.015) 4px)' }} />
      <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', background: 'radial-gradient(ellipse at 50% 50%, rgba(94,234,212,0.04) 0%, transparent 70%)' }} />

      <div style={{ width: '100%', maxWidth: 340, position: 'relative', zIndex: 1 }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 52, height: 52, border: `1px solid ${BORDER}`, background: 'var(--surface-1)', marginBottom: '0.875rem', boxShadow: GLOW }}>
            <svg width="26" height="26" fill={C} viewBox="0 0 28 28" style={{ filter: 'drop-shadow(0 0 4px rgba(94,234,212,0.8))' }}>
        ...
        <div style={{ background: 'var(--surface-1)', border: `1px solid ${BORDER}`, padding: '2rem', boxShadow: '0 0 30px rgba(94,234,212,0.08), inset 0 0 30px rgba(94,234,212,0.02)' }}>
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            <div style={{ fontSize: '0.875rem', fontWeight: 700, color: C, textShadow: '0 0 6px rgba(94,234,212,0.6)' }}>{'> '}authenticate</div>
          </div>

          {error && (
            <div style={{ border: '1px solid rgba(255,51,51,0.4)', background: 'rgba(255,51,51,0.06)', color: 'var(--danger)', padding: '0.5rem 0.75rem', fontSize: '0.75rem', marginBottom: '1.25rem', display: 'flex', gap: '0.5rem', boxShadow: '0 0 8px rgba(255,51,51,0.2)' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div>
              ...
              <input
                ...
                style={{ width: '100%', padding: '0.5625rem 0.75rem', background: 'var(--bg)', border: `1px solid ${BORDER}`, color: C, fontFamily: 'inherit', fontSize: '0.875rem', outline: 'none', caretColor: C, boxSizing: 'border-box' }}
            ...
            <button
              ...
              onMouseEnter={e => { if (!loading) { (e.target as HTMLElement).style.background = C; (e.target as HTMLElement).style.color = '#000'; (e.target as HTMLElement).style.boxShadow = '0 0 20px rgba(94,234,212,0.8), 0 0 40px rgba(94,234,212,0.4)' } }}
