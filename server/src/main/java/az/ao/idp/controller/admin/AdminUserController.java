...
public class AdminUserController {
    ...
    public AdminUserController(UserService userService, LdapService ldapService) {
        this.userService = userService;
        this.ldapService = ldapService;
    }

    @GetMapping("/ldap/tree")
    public ResponseEntity<List<LdapTreeNode>> ldapTree(@RequestParam(required = false) String dn) {
        return ResponseEntity.ok(ldapService.listLdapChildren(dn, userService.getAllActivatedLdapUsernames()));
    }

    @GetMapping("/ldap/ous")
    public ResponseEntity<List<LdapService.LdapOuInfo>> ldapOus() {
        return ResponseEntity.ok(ldapService.listOus());
    }

    @GetMapping("/ldap/users")
    public ResponseEntity<List<LdapUserResponse>> ldapUsers(
            @RequestParam(required = false) String search,
            @RequestParam(required = false) String dn) {
        return ResponseEntity.ok(ldapService.listUsers(dn, search).stream().map(u ->
                new LdapUserResponse(u.ldapUsername(), u.email(), u.displayName(),
                        userService.userExistsByLdapUsername(u.ldapUsername()),
                        u.title(), u.ou(), u.groups())
        ).toList());
    }
    ...
    public ResponseEntity<User> activate(@Valid @RequestBody ActivateUserRequest request) {
        LdapService.LdapUserAttributes attrs = ldapService.getUserAttributes(request.ldapUsername());
    ...
    public ResponseEntity<User> activateForApp(
        ...
        LdapService.LdapUserAttributes attrs = ldapService.getUserAttributes(ldapUsername);
